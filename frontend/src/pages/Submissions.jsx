...
export default function Submissions(){
  ...
  const { worker, logout } = useBilling()
  ...
  async function doDelete(id, deletedItem){
    try{
      const res = await fetch(apiUrl(`/submissions/${encodeURIComponent(id)}`), { method: 'DELETE' })
      const text = await res.text()
      ...
      setSnack({ open:true, message:'Deleted', actionLabel:'Undo', onAction:async ()=>{
        if(deletedItem){
          try{
            await fetch(apiUrl('/submissions'), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(deletedItem) })
  ...
  async function saveEdit(){
    try{
      const res = await fetch(apiUrl(`/submissions/${encodeURIComponent(editingId)}`), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editValues) })
      const text = await res.text()
